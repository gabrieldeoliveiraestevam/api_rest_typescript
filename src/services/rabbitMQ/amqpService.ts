...
import amqp, { AmqpConnectionManager, Channel, Options } from 'amqp-connection-manager';
...
class AMQPService {
    ...
    private setup(): void {
        ...
        this.channel = this.connection.createChannel({
            setup: async (channel: Channel) => {
              console.log('Conectado ao RabbitMQ. Realizando configurações das exchanges e filas.');
    ...
    private async setupQueuesAndExchanges(channel: Channel): Promise<void> {
        this.channel.assertExchange('exchange-student-grade', 'direct', this.assertExchangeOptions);
